 ...
 * * you may use csci373::allocation_table.summary();
 * *
 * * You must simply indicate that you want to start recording by
 * * using csci373::allocation_table.summary();
 * *
 * **************************************************************************
 ****************************************************************************/
 ...
 * Copyright (c) 2006-13 by Art Diky
 *
 * DO NOT EDIT THIS FILE
 *
 * This file is broken into two parts:
 *
 *   1. The first namespace csci373 is associated with tools used by the
 *      second part.
...
#define ALLOCATION_TABLE_SIZE 8192
...
namespace csci373 {
    ...
    class invalid_deletion : public std::logic_error {
    public:
        explicit invalid_deletion (const std::string& what_arg) : logic_error(what_arg) {} ;
    };

    class allocator_limit : public std::exception {
    public:
        const char* what() const throw() {
            return "Allocating more memory than is allowed for this project. Allocations allowed: " TOSTRING(ALLOCATION_TABLE_SIZE);
        }
    };
    ...
    class Stopwatch {
        ...
        public:
            ...
            void stop() {
                ...
                duration = std::chrono::duration_cast<T>(end_time - start_time).count();
            }
    ...
    class HashTable {
        ...
        public:
            ...
            void reserve( int N ) {
                ...
                if ( (N & ((~N) + 1)) != N ) {
                    throw std::invalid_argument("Incorrect argument value. Must be a power of 2");
                }
            ...
            void insert( void *ptr, size_t size, bool is_array ) {
                ...
                for ( int i = 0; i < array_size; ++i ) {
                    ...
                    if ( allocated[hash].address == 0 || allocated[hash].address == ptr ) {
                        ...
                        total_memory_alloc += static_cast<int>(size);
            ...
            size_t remove( void *ptr, bool is_array ) {
                    ...
                    if ( allocated[hash].address == ptr ) {
                        ...
                        if ( allocated[hash].is_array  != is_array ) {
                            if ( allocated[hash].is_array ) {
                                err << "Use 'delete [] ptr;' to free memory allocated with 'ptr = new Class[array_size];'" << std::endl;
                            } else {
                                err << "Use 'delete ptr;' to free memory allocated with 'ptr = new Class(...);'" << std::endl;
                            }
                            throw invalid_deletion(err.str());
                        } else if ( allocated[hash].deleted ) {
                            err << "Calling delete twice on the same memory location: " << ptr << std::endl;
                            throw invalid_deletion(err.str());
                        }

                        allocated[hash].deleted = true;
                        total_memory_deleted += static_cast<int>(allocated[hash].size);
                ...
                err << "Deleting a pointer to which memory was never allocated: " << ptr << std::endl;
                throw invalid_deletion(err.str());
            }
    ...
    HashTable allocation_table( ALLOCATION_TABLE_SIZE );
    ...
    // When the user returns memory, we must check that the padding is unchanged.
    //    ________________                                            ________________
    ...
    void check_array_bounds( char *ptr, size_t size ) {
        ...
        for ( size_t i = 0; i < PAD; ++i ) {
            if ( ptr[i] != 'U' ) {
                err << "Memory before the array located at address "
                    << static_cast<void *>( ptr + PAD ) << " was overwritten" << std::endl;
                throw std::out_of_range(err.str());
            }

            if ( ptr[size - 1 - i] != 'U' ) {
                err << "Memory after the array located at address "
                    << static_cast<void *>( ptr + PAD ) << " was overwritten" << std::endl;
        ...
        if ( !used ) {
            std::cout << "The memory allocated at address "
...
void *operator new( size_t size ) {
    ...
    try {
        csci373::allocation_table.insert( ptr, size, false );
    }
    catch (const csci373::allocator_limit &e) {
        std::unexpected();
    }
...
void operator delete( void *ptr ) {
    try {
        csci373::allocation_table.remove( ptr, false );
    }
    catch (const std::out_of_range &e) {
        std::cout << "WARNING: " << e.what() << std::endl;
    }
    catch (const csci373::invalid_deletion &e) {
        std::cout << "WARNING: " << e.what() << std::endl;
    }
    free( ptr );
}
...
void *operator new[]( size_t size ) {
    char *ptr = static_cast<char *>( malloc( size + 2*csci373::PAD ) );
    if (ptr == nullptr) {
        throw std::bad_alloc();
    }
    try {
        csci373::allocation_table.insert( static_cast<void *>( ptr + csci373::PAD ), size, true );
        csci373::initialize_array_bounds( ptr, size + 2*csci373::PAD );
    }
    catch (const csci373::allocator_limit &e) {
        std::unexpected();
    }
    return static_cast<void *>( ptr + csci373::PAD );
}
...
void operator delete[]( void *ptr ) {
    size_t size = csci373::allocation_table.remove( ptr, true );
    try {
        if ( csci373::allocation_table.is_recording() ) {
            csci373::check_array_bounds( static_cast<char *>( ptr ) - csci373::PAD, size + 2*csci373::PAD );
        }
    }
    catch (const std::out_of_range &e) {
        std::cout << "WARNING: " << e.what() << std::endl;
    }
    free( static_cast<char *>( ptr ) - csci373::PAD );
}
